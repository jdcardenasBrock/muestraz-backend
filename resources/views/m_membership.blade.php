...
@section('content')
    ...
    <div class="membership-container pt-4">
        @foreach ($membership as $index => $m)
            ...
            <form method="POST" action="{{ route('membership.pay') }}"> @csrf <input type="hidden" name="membership_id"
                    value="{{ $m->id }}">
                <div class="membership-card {{ $cardClass }} {{ $isFeatured }}">
                    @if ($isFeatured)
                        <div class="ribbon">Popular</div>
                        @endif <div class="card-image"> <img src="{{ Storage::url($m->image_path) }}"
                                alt="{{ $m->type }}"> </div>
                        <h2 class="card-title">{{ ucfirst($m->type) }}</h2>
                        <p class="card-price"> {{ $m->value > 0 ? '$' . number_format($m->value, 2) : 'Gratis' }} </p>
                        ...
                        <ul class="card-features">
                            ...
                            @else
                                ...
                                @endif @if ($m->value == 0 || $m->type == 'free')
                        ...
                        @else
                            <button class="card-button" type="submit">La Quiero</button>
                        @endif
                </div>
            </form>
        @endforeach
